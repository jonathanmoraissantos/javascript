...
function start() {
    ...
    if (hora < 6 || hora >= 23) {
        fundo.style.backgroundImage = 'linear-gradient(to bottom, #0D0D00, #1D2C40)'
        imagem.style.backgroundImage = "url(images/hora/madrugada.png)"
    } else if (hora < 12) {
        fundo.style.background = 'linear-gradient(to bottom, #F2CF8D, #D9C8A9)'
        imagem.style.backgroundImage = "url(images/hora/manha.png)"
    } else if (hora <= 18) {
        fundo.style.backgroundImage = 'linear-gradient(to bottom, #0378A6, #DFE7F2)'
        imagem.style.backgroundImage = "url(images/hora/tarde.png)"
    } else {
        fundo.style.backgroundImage = 'linear-gradient(to bottom, #734002, #BFB188)'
